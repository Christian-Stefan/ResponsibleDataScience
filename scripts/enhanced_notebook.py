...
This script provides a comprehensive analysis of ICU patient data to predict mortality,
including advanced visualization, model explanations, and fairness evaluations.
"""
...
def main():
    ...
    print("Starting ICU Mortality Prediction Analysis...")
    ...
    try:
        ...
        try:
            ...
            if model_files:
                ...
                print("Training new models with progress bar...")
            ...
            if fairness_df is not None:
                ...
                # Train a fairness constrained model as an alternative
                ...
                try:
                    ...
                    # Compare original and constrained models
                    ...
                    # Calculate fairness metrics for constrained model
            ...
            # Ensure age column is numeric before creating age groups
            ...
            print("\nAnalyzing feature importance with permutation importance (this may take a while)...")
            try:
                # Create a progress bar context for permutation importance
                with tqdm(total=100, desc="Permutation importance", unit="%") as pbar:
                    # Define callback for progress updates
                    def update_progress(iteration, total):
                        pbar.update(int(100 * iteration / total) - pbar.n)
            ...
            try:
                ...
                # Calculate partial dependence for top numerical features
                ...
                if numerical_top_features:
                    ...
                    for feature in tqdm(numerical_top_features, desc="Calculating partial dependence"):
                        try:
                            ...
                            print(f"Error calculating partial dependence for {feature}: {str(e)}")
                            continue
                    
                    print("Partial dependence analysis completed successfully")
                else:
                    print("No numerical features found in top features for partial dependence analysis")
